...
public interface RepositorioRegistroVehiculoJPA extends CrudRepository<RegistroVehiculoEntity, Integer>{
    ...
    @Query("SELECT CASE WHEN COUNT(rv.id) > 0 THEN true ELSE false END FROM RegistroVehiculoEntity rv WHERE rv.placa = :placa AND rv.tipoid = :tipoid AND rv.salidae is null")
    public boolean existe(@Param("placa") String placa, @Param("tipoid") int tipoId);
    
    @Query("SELECT COUNT(rv.id) FROM RegistroVehiculoEntity rv WHERE rv.tipoid = :tipo AND rv.salidae is null")
    public int validarCuposPorTipoVehiculo(@Param("tipo") int tipo);

    @Query("SELECT rv FROM RegistroVehiculoEntity rv WHERE rv.placa = :placa AND rv.salidae is null AND rv.totale = 0")
    public RegistroVehiculoEntity obtenerRegistroVehiculoPorPlaca(@Param("placa") String placa);
    
    @Query("SELECT rv FROM RegistroVehiculoEntity rv WHERE rv.salidae is null AND rv.totale = 0")
